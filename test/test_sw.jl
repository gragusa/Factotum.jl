@testset "Stock-Watson LS factor model" begin
    ...
    ftm = Factotum.FactorModel(
        X_full, 6; scale = true, method = :ls, orthonormalize = false)

    # Expected values from raw LS solution (no QR orthonormalization)
    expected_factors = [-1.8682 -0.3544 -0.2306 -0.0071 1.7681 -0.9233
    ...
    expected_loadings = [-0.00892726 -0.00608655 0.0435287 -0.00612441 -0.00356542 -0.00157926
    ...
    @testset "Loading values (last 4 rows)" begin
        @test Factotum.loadings(ftm; original_units = true)[(end - 3):end, :] ≈
              expected_loadings atol=1e-5
    end
end
